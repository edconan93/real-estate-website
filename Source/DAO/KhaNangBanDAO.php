...
<?php
	class KhaNangBanDAO
	{
    	public static function Add ($ten, $status)
        {
            $strSQL = "Insert into khanangban values (NULL,'$ten', $status)";
    	...
    	public static function Update ($id, $ten, $status)
        {

            $strSQL = "update khanangban set ten='$ten', status=$status
					   where id=$id";
		    $cn = DataProvider::Open ();
			DataProvider::MoreQuery ($strSQL,$cn);
        ...
        {
            $strSQL = "update khanangban set status= '$status' where user_id=$user_id";
            $cn = DataProvider::Open ();
			DataProvider::MoreQuery ($strSQL,$cn);
